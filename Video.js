function video(backgroundCanvas, spriteBackgroundCanvas, foregroundCanvas, spriteforegroundCanvas, mem, cpu) {
  ...
  function drawTextModeNormal(charPos) {
    ...
    var extendedModeEnabled = (registers[0x11] & 0x40) != 0;
    var extendedModeBackgroundIndex;
    if (extendedModeEnabled) {
      extendedModeBackgroundIndex = (screenCode & 0xc0) >> 6; 
      screenCode = screenCode & 0x3f;
    }
    var currentLine = localMem.vicRead(baseCharAdd + (screenCode << 3) + ((cycleline - 42) & 7));
    var textColor = colorRAM[charPos] & 0xf;
    var backgroundColor;
    if (!extendedModeEnabled) {
      backgroundColor = registers[0x21] & 0xf;
    } else {
      backgroundColor = registers[extendedModeBackgroundIndex + 0x21] & 0xf;
    }
